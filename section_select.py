...
targ_sections = ["addendum","service","hospital cource","brief resume of hospital course","brief hospital cource","diagnosis","past history of illness","present history of illness","physical examination","physical exam upon transfer to the gms service","disposition"]#"medication disposition" #laboratory
threshold = 0.63
...
for num,txt in enumerate(txts):
    ...
    for i in range(len(match_result)):
        ...
        #match_result[i]=match_result[i].replace(":","")
        match_result[i]=match_result[i].lower()
    for section in final_sections:
        if section in match_result:
            txt_index=temp_txt.find(section)
            section_txt=""
            section_index=match_result.index(section)
            if section_index+1<len(match_result):
                txt_next_index=temp_txt.index(match_result[section_index+1])
            else:
                txt_next_index=len(txt)-1
            for i in range(txt_index+len(section)+1,txt_next_index):
                section_txt+=txt[i]
